...
const orderTermStyle = "w-full text-left transition hover:scale-110 font-mono";
const selectedTermStyle = "w-full px-2 text-left border-l-5";
...
function FilterSide(props: {
    ...
    selectedOrderTermIndex: number, 
    setSelectedOrderTermIndex: Dispatch<SetStateAction<number>>
}) {
    const highlightButton = (index: number, orderTerm: string) => {
        props.setSelectedOrderTermIndex(index);
        ...
        <ResponsiveDiv style="" children={<>
            <ResponsiveDiv style="w-full flex flex-row justify-between mb-1" children={<>
                ...
                <ResponsiveDiv style="flex flex-row" children={<>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6" onClick={() => props.setArrangementType("grid")}>
                        <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 6A2.25 2.25 0 0 1 6 3.75h2.25A2.25 2.25 0 0 1 10.5 6v2.25a2.25 2.25 0 0 1-2.25 2.25H6a2.25 2.25 0 0 1-2.25-2.25V6ZM3.75 15.75A2.25 2.25 0 0 1 6 13.5h2.25a2.25 2.25 0 0 1 2.25 2.25V18a2.25 2.25 0 0 1-2.25 2.25H6A2.25 2.25 0 0 1 3.75 18v-2.25ZM13.5 6a2.25 2.25 0 0 1 2.25-2.25H18A2.25 2.25 0 0 1 20.25 6v2.25A2.25 2.25 0 0 1 18 10.5h-2.25a2.25 2.25 0 0 1-2.25-2.25V6ZM13.5 15.75a2.25 2.25 0 0 1 2.25-2.25H18a2.25 2.25 0 0 1 2.25 2.25V18A2.25 2.25 0 0 1 18 20.25h-2.25A2.25 2.25 0 0 1 13.5 18v-2.25Z" />
                    </svg>
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-6" onClick={() => props.setArrangementType("row")}>
            ...
            <ResponsiveDiv style="flex flex-col items-start mt-2 gap-2 mb-1" children={<>
                <MonoStyleText style={filterLableStype} content="ORDER BY" />
                {props.orderTerms.map((orderTerm : string, index : number) => {
                    return (
                        <button id={props.orderTerms[index]} key={index} className={index == props.selectedOrderTermIndex ? selectedTermStyle : orderTermStyle} onClick={() => {           
                            highlightButton(index, orderTerm);
                        }} >
                            {orderTerm}
                        </button>
                    )
