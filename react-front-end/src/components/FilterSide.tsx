...
const unselectedStyle = "bg-white text-black transition duration-300 ease-in-out hover:-translate-y-1 hover:scale-110 hover:bg-indigo-500 hover:text-white";
...
function FilterSide(props: {
    ...
    const orderTermButtonClassNames = useRef(new Array<string>(props.orderTerms.length).fill(
        "bg-white text-black transition duration-300 ease-in-out hover:-translate-y-1 hover:scale-110 hover:bg-indigo-500 hover:text-white"
    ));

    return (
        <ResponsiveDiv style="flex flex-col" children={[
            <ResponsiveDiv style="flex flex-row justify-between" children={[
                <p className="w-30 texf-xl text-black">DISPLAY</p>,
            ...
            <ResponsiveDiv style="flex flex-col mt-2 gap-2" children={[
                <p className="w-30 texf-xl text-black mb-2">ORDER</p>, 
                props.orderTerms.map((orderTerm : string, index : number) => {
                    return (
                        <button key={index} className={orderTermButtonClassNames.current[index]} onClick={() => {
                            orderTermButtonClassNames.current = orderTermButtonClassNames.current.map((_ : string, idx: number) => {
                                if (index == idx) {
            ...
            <ResponsiveDiv style="flex flex-col mt-2" children={[
                <p className="w-30 texf-xl text-black mb-2">FILTER</p>,
                <ResponsiveDiv style="flex flex-row gap-2" children={[
                    <label>PRICE:</label>,
                    <p>$</p>,
                    <input className="border-1 w-15" type="text"></input>,
                    <p> - </p>,
                    <p>$</p>,
                    <input className="border-1 w-15" type="text"></input>,
                ]} />
